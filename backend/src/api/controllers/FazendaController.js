...
class FazendaController {
  ...
  async show_vacas_fazenda(request, response) {
    const fazendaId = request.params.idFazenda;

    try {
      const bovinos = await bovinoService.getAllBovinoFem(fazendaId);
      return response.status(200).json(bovinos);
    } catch (error) {
      return response.status(400).json({
        error: error.message,
  ...
  async show_empregados_fazenda(request, response) {
    const { idFazenda } = request.params;
    try {
      const empregado = await EmpregadosService.getEmpregadoIdFazenda(
        idFazenda
      );
